...
while 1
    ...
    if and(code_sat, code_rng)==1 && count==2
        ...
        if i==1
            ...
            [estuser(i,1:4), dp{i,1}] = LS_pos(50, satpos{i}, pseudo{i}, corr{i});
            estpos(i,1:3)=ecef2lla(estuser(i,1:3));
            [prc{i,1}, geoP{i,1}]=calPRC(estuser(1,1:3), satpos{i}, pseudo{i}, corr{i});
            
            fprintf('1번째 위치 (Rover) [%d-%02d-%02d %02d:%02d:%02d] : %.10f %.10f %.2fm\n', time_SATXYZ2(1,1), time_SATXYZ2(1,2), time_SATXYZ2(1,3), time_SATXYZ2(1,4), time_SATXYZ2(1,5), time_SATXYZ2(1,6), estpos(1,1), estpos(1,2), estpos(1,3));
        ...
        elseif i > 1
            ...
            [estuser(i,1:4), dp{i,1}] = LS_pos(50, satpos{i}, pseudo{i}, corr{i}, estuser(i-1,1:4));
            estpos(i,1:3)=ecef2lla(estuser(i,1:3));
            [prc{i,1}, as{i,1}]=calPRC(estuser(1,1:3), satpos{i}, pseudo{i}, corr{i});
            
            fprintf('%d번째 위치 (Rover) [%d-%02d-%02d %02d:%02d:%02d] : %.10f %.10f %.2fm\n', i, time_SATXYZ2(i,1), time_SATXYZ2(i,2), time_SATXYZ2(i,3), time_SATXYZ2(i,4), time_SATXYZ2(i,5), time_SATXYZ2(i,6), estpos(i,1), estpos(i,2), estpos(i,3));
    ...
    if and(code_sat, code_rng)~=1 && count==2
        ...
        [estuser(i,1:4), dp{i,1}] = LS_pos(20, satpos{i-1}, pseudo{i-1}, corr{i-1}, estuser(i-2,1:4));
        estpos(i,1:3)=ecef2lla(estuser(i-1,1:3));
        [prc{i,1}, as{i,1}]=calPRC(estuser(1,1:3), satpos{i-1}, pseudo{i-1}, corr{i-1});
...
save('rover', 'xyz_BESTXYZ', 'estuser', 'satpos', 'pseudo', 'corr', 'geoP', 'dp');
fprintf('출력 완료\n')
